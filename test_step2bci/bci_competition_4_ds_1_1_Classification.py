...
"""
targets: -1: Imagination de la main gauche (Left) = pieds , 1: Imagination de la main droite (Right)
"""
...
freqs, welch_r = analysis.welch_periodogram_analysis(inputData=trials_csp['right'], sliding_win=200, sfreq=sfreq, t_ax=1)

trials_PSD = {'left': welch_f, 'right': welch_r}

plotter.trials_psd_plot(trials_PSD, freqs, chan_ind=[0, 28, -1],
                        chan_lab=['1ère Comp.', 'Comp. centrale', 'Dernière Comp.'], maxy=0.75, xlabel='Fréquence (''$Hz$)',
...
scores, y_pred = tools.estimator_select(None, None, cv=5, ts=0.25, rs=1234, splitted_data=splitted_data,
                                        with_predict=True)
print(scores)
